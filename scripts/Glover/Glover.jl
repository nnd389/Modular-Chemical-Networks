# NOTE FROM NINA: The script below implements the chemical reaction network written by Glover in 2010. 
# Below you will see a 219 reaction network and thanks to catalyst, you get to see the network in reaction form!
# Catalyst will do the work of converting the reactions to an ODE system for you :)

# The Glover (2010) paper has exactly 218 reaction in their appendix, 
# but when I was implementing this network I found it was producing some pretty funky results. 
# Turns out it was missing one very important reaction, which was the reverse reaction for reaction number 20. 
# You can find this one added reaction if you control f "k20_reverse"
...
tspan = (0.0, 1000000 * seconds_per_year) 
...
u0_neutral = [  0.0,# 1: H 
    ...
    ### Parameters ###
T = 300 # from glover paper, T=60 section 4, nelson has 10k
Td = 300
Te = 0.02585198444922 # 300k = 0.02585198444922
n_H = 611 
Av = 2 # Glover Section 2 gives formula: n_H/(1.87e21)
# can calculate AV with av = baseav +col_dens / 1.6e21
cr_ion_rate = 6.1818e-18 # Nelson has 6.18e-18 baked into their network,   I tried 6e-18, DESPOTIC suggests 2.0e-16
...
sys_glover = convert(ODESystem, complete(system))
comp_glover = complete(sys_glover)
glover_colors = palette(:devon, 10)


prob_fiducial = ODEProblem(comp_glover, u0_fiducial, tspan, params)
sol_fiducial = solve(prob_fiducial, lsoda(), reltol=1.49012e-8, abstol=1.49012e-8, saveat=9.4608e10)
plot(sol_fiducial, vars = (0,10), lw = 3, lc = glover_colors[1])

prob_neutral = ODEProblem(comp_glover, u0_neutral, tspan, params)
sol_neutral = solve(prob_neutral, lsoda(), reltol=1.49012e-8, abstol=1.49012e-8, saveat=9.4608e10)
plot!(sol_neutral, vars = (0,10), lw = 3, lc = glover_colors[5])



    ### For saving results to an excel sheet ###
#=
using DataFrames
using XLSX
species_names = ["H", "e", "H-", "H2", "H+", "H2+", "He", "He+", "C+", "C", "O+", "O", "OH", "HOC+", "HCO+", "CO", "CH", "CH2", "C2", "H2O", "O2", "H3+", "CH+", "CH2+", "CO+", "CH3+", "OH+", " H2O+", "H3O+", "O2+", " C-", "O-", "M"]
...
XLSX.openxlsx("/Users/kneenaugh/Desktop/Git/AstroChemNetwork/scripts/Data/glover_comparison.xlsx", mode="w") do xf
