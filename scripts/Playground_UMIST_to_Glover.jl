...
params = Dict(
    ...
    :k154 => n_H^2 * 1.32e-32 * (T/300)^-0.38,
    :k157 => n_H^2 * 5.99e-33 * (T/5000)^(-1.6), 
    :k158 => n_H^2 * 6.16e-29 * (T/300)^(-3.08),
...
# Put n_H in front of all these and n_H^2 if k154-k163
if T > 6000
...
# %% Define the network
reaction_equations = [
    (@reaction k1, H + e --> H⁻),    #k1
    (@reaction k2, H⁻ + H --> H2 + e), #k2
    ...
    (@reaction n_H *  5.26e-18 *(T/300)^(-5.22) * exp(-90/T) , OH + H --> H2O ), # k153: Glover rate is 
    
    # Reactions with three reactants (k154-k163) start below
    (@reaction k154, H + H + H --> H2 + H), # k154: SAME, Umist doesnt have these
    (@reaction n_H^2 * 2.8e-31 * (T)^-0.6 , H + H + H2 --> H2 + H2), # k155: SAME, Umist doesnt have these
    (@reaction n_H^2 * 6.9e-32 * (T)^(-0.4) , H + H + He --> H2 + He), # k156: SAME, Umist doesnt have these
    (@reaction k157, C + C + M --> C2 + M), # 157: SAME, Umist doesnt have these
    (@reaction k158, C + O + M --> CO + M), # k158: SAME, Umist doesnt have these
    (@reaction n_H^2 * 100 * cr_ion_rate * 960, C⁺ + O + M --> CO⁺ + M), # k159: SAME, Umist doesnt have these FIX: these rely on k210, which doesn't exist?
    (@reaction n_H^2 * 100 * cr_ion_rate * 960, C + O⁺ + M --> CO⁺ + M), # k160: SAME, Umist doesnt have these FIX: these rely on k210, which doesn't exist?
    (@reaction n_H^2 * 4.33e-32 * (T/300)^(-1), O + H + M --> OH + M), # k161: SAME, Umist doesnt have these
    (@reaction n_H^2 * 2.56e-31 * (T/300)^(-2), OH + H + M --> H2O + M), # k162: SAME, Umist doesnt have these
    (@reaction n_H^2 * 9.2e-34 * (T/300)^(-1), O + O + M --> O2 + M), # k163: SAME, Umist doesnt have these
    
    (@reaction n_H * 1.09e-11 * (T/300)^(-2.19) * exp(-(165.1/T)), O + CH -->  HCO⁺ + e), # k164: Glover rate is n_H * 2e-11 * (T/300)^(0.44) 
    #(@reaction 3e-18 * (T)^(0.5) * (1 + 1e4 * exp(-600/Td))^(-1) * (1 + 0.04*(T + Td)^(0.5) + 0.002*T + 8e-6 * T^2)^(-1) , H+H --> H2), # FIX H is H(s) here?
    

    # Photochemical Reactions (R166) start below
    (@reaction 1.43e-07 * exp(-(0.5*Av)), H⁻ --> H + e), # k166: Glover rate is 7.1e-7 * exp(-0.5 * Av)
    (@reaction 5.7e-10 * exp(-(2.4*Av)), H2⁺ --> H + H⁺), # k167: Glover rate is 1.1e-9 * exp(-1.9 * Av)
    (@reaction 1.3e-18, H2   -->  H + H), # k168: Glover rate is 5.6e-11 * exp(-2.55*Av + 0.0165*(Av)^2)
    (@reaction 5e-15 * exp(-(1.8*Av)), H3⁺ --> H2 + H⁺), # k169: Glover rate is 4.9e-13 * exp(-1.8 * Av)
    (@reaction 5e-15 * exp(-(2.3*Av)), H3⁺ --> H2⁺ + H), # k170: Glover rate is 4.9e-13 * exp(-2.3 * Av)
    (@reaction 3.1e-10 * exp(-(3.3*Av)), C -->  C⁺ + e), # k171: Glover rate is 3.1e-10 * exp(-3.0 * Av)
    (@reaction 4.9e-08 * exp(-(0.5*Av)), C⁻ --> C + e), # k172: Glover rate is 2.4e-7 * exp(-0.9 * Av)
    (@reaction 9.2e-10 * exp(-(1.7*Av)), CH --> C + H), # k173: Glover rate is 8.7e-10 * exp(-1.2 * Av)
    (@reaction 7.6e-10 * exp(-(3.3*Av)), CH --> CH⁺ + e), # k174: Glover rate is 7.7e-10 * exp(-2.8 * Av)
    (@reaction 3.3e-10 * exp(-(2.9*Av)), CH⁺ --> C + H⁺), # k175: Glover rate is 2.6e-10 * exp(-2.5 * Av)
    (@reaction 5.8e-10 * exp(-(2.0*Av)), CH2 --> CH + H), # k176: Glover rate is 7.1e-10 * exp(-1.7 * Av)
    (@reaction 1e-09 * exp(-(2.3*Av)), CH2 --> CH2⁺ + e), # k177: Glover rate is 5.9e-10 * exp(-2.3 * Av)
    (@reaction 4.67e-11 * exp(-(2.2*Av)), CH2⁺ --> CH⁺ + H), # k178: Glover rate is 4.6e-10 * exp(-1.7 * Av)
    (@reaction 1e-09 * exp(-(1.7*Av)), CH3⁺ --> CH2⁺ + H), # k179: Glover rate is 1.0e-9 * exp(-1.7 * Av)
    (@reaction 1e-09 * exp(-(1.7*Av)), CH3⁺ --> CH⁺ + H2), # k180: Glover rate is 1.0e-9 * exp(-1.7 * Av)
    (@reaction 2.4e-10 * exp(-(2.6*Av)), C2 --> C + C), # k181: Glover rate is 1.5e-10 * exp(-2.1 * Av)
    (@reaction 1.09e-08 * exp(-(0.5*Av)), O⁻ --> O + e), # k182: Glover rate is 2.4e-7 * exp(-0.5 * Av)
    (@reaction 3.9e-10 * exp(-(2.2*Av)), OH --> O + H), # k183: Glover rate is 3.7e-10 * exp(-1.7 * Av)
    (@reaction 1.6e-12 * exp(-(3.1*Av)), OH --> OH⁺ + e), # k184 Glover rate is 1.6e-12 * exp(-3.1 * Av)
    #(@reaction 1.0e-12 * exp(-1.8 * Av), OH⁺  --> O + H⁺), 
    (@reaction 8e-10 * exp(-(2.2*Av)), H2O --> OH + H), # k186: Glover rate is 6.0e-10 * exp(-1.7 * Av)
    (@reaction 3.1e-11 * exp(-(3.9*Av)), H2O --> H2O⁺ + e), # k187: Glover rate is 3.2e-11 * exp(-3.9 * Av)
    #(@reaction R188, H2O⁺ --> H2⁺ + O), 
    #(@reaction R189, H2O⁺ --> H⁺ + OH), 
    #(@reaction R190, H2O⁺ --> O⁺ + H2), 
    (@reaction 1e-12 * exp(-(2.0*Av)), H2O⁺ --> OH⁺ + H), # k191: Glover rate is R191
    #(@reaction R192, H3O⁺ --> H⁺ + H2O), 
    #(@reaction R193, H3O⁺ --> H2⁺ + OH), 
    #(@reaction R194, H3O⁺ --> H2O⁺ + H), 
    #(@reaction R195, H3O⁺ --> OH⁺ + H2), 
    (@reaction 7.6e-11 * exp(-(3.9*Av)), O2 --> O2⁺ + e), # k196: Glover rate is 5.6e-11 * exp(-3.7 * Av)
    (@reaction 7.9e-10 * exp(-(2.1*Av)), O2 --> O + O), # k197: Glover rate is 7.0e-10 * exp(-1.8 * Av)
    (@reaction 2e-10 * exp(-(3.5*Av)), CO   --> C + O), # k198: Glover rate is: 2e-10 * exp(-3.5 * Av) 


    # Cosmic Ray Reactions (R199) start below 
    (@reaction 5.98e-18, H --> H⁺ + e), # k199: Glover rate is cr_ion_rate * 1 
    (@reaction 6.5e-18, He --> He⁺ + e), # k200: Glover rate is cr_ion_rate * 1.1333333 
    (@reaction 2.86e-19, H2 --> H⁺ + H + e), # k201: Glover rate is cr_ion_rate * 0.037 
    (@reaction 1.3e-18, H2 --> H + H), # k202: Glover rate is cr_ion_rate * 0.22 
    (@reaction 3.9e-21, H2 --> H⁺ + H⁻), # k203: Glover rate is cr_ion_rate * 6.5e-4 
    (@reaction 2.3e-17, H2 --> H2⁺ + e), # k204: Glover rate is cr_ion_rate * 2 
    (@reaction 2.3e-17, C --> C⁺ + e), # k205: Glover rate is cr_ion_rate * 3.8 
    (@reaction 3.4e-17, O --> O⁺ + e), # k206: Glover rate is cr_ion_rate * 5.7 
    (@reaction 3.9e-17, CO --> CO⁺ + e), # k207: Glover rate is cr_ion_rate * 6.5 
    (@reaction 1.3e-17 * (T/300)^(0.0) * (255.0/(1-omega)), C --> C⁺ + e), # k208: Glover rate is cr_ion_rate * 2800 
    (@reaction 1.3e-17 * (T/300)^(0.0) * (365.0/(1-omega)), CH --> C + H), # k209: Glover rate is cr_ion_rate * 4000 
    (@reaction 1.3e-17 * (T/300)^(0.0) * (88.0/(1-omega)), CH⁺ --> C⁺ + H), # k210 Glover rate is cr_ion_rate * 960 
    (@reaction 1.3e-17 * (T/300)^(0.0) * (250.0/(1-omega)), CH2 --> CH2⁺ + e), # k211: Glover rate is cr_ion_rate * 2700 
    (@reaction 1.3e-17 * (T/300)^(0.0) * (250.0/(1-omega)), CH2 --> CH + H), # k212: Glover rate is cr_ion_rate * 2700 
    (@reaction 1.3e-17 * (T/300)^(0.0) * (119.5/(1-omega)), C2 --> C + C), # k213: Glover rate is cr_ion_rate * 1300 
    (@reaction 1.3e-17 * (T/300)^(0.0) * (254.5/(1-omega)), OH --> O + H), # k214: Glover rate is cr_ion_rate * 2800 
    (@reaction 1.3e-17 * (T/300)^(0.0) * (485.5/(1-omega)), H2O --> OH + H), # k215: Glover rate is cr_ion_rate * 5300 
    (@reaction 1.3e-17 * (T/300)^(0.0) * (375.5/(1-omega)), O2 --> O + O), # k216: Glover rate is cr_ion_rate * 4100 
    (@reaction 1.3e-17 * (T/300)^(0.0) * (58.5/(1-omega)), O2 --> O2⁺ + e), # k217: Glover rate is cr_ion_rate * 640  
    (@reaction 1.3e-17 * (T/300)^(1.17) * (105.0/(1-omega)), CO --> C + O) # k218: Glover is .21*T^(1.2) * x_H2 * x_CO^(-1/2)???
]
...
# C and C+
plot(sol, idxs = (0,10), lw = 3, lc = "blue", title = "Glover with Umist glover rates mix")
plot!(sol, idxs = (0,9), lw = 3, lc = "orange", title = "Glover with Umist glover rates mix")
