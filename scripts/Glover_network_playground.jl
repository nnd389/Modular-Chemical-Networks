...
tspan = (0.0, .03 * seconds_per_year) # ~30 thousand yrs
...
# THIS INITAIL CONDITIONS IS WRONG, I trying to see if I can reporduce Nelson
u0 = [  0,        # 1:-
        2e-4,     # 2:-
        0,        # 3:-
        2e-4,     # 4:-
...
reaction_equations = [
    ...
    (@reaction n_H * k20, C⁺ + e --> C ), # Nelson Match Electron-Recombination reaction 3, Nelson has 1.4e-10 * (T)^(-0.61)
    ...
    # (@reaction n_H * 2.05e-12 * (T/300)^(1.52) * exp(-1736/T) , OH + H2 --> H2O + H), # FIX with rate
    ...
    (@reaction n_H * 2.34e-8 * (T/300)^(-0.52) , H3⁺ + e --> H2 + H), # Nelson Match Electron-Recombination reaction 2
    ...
    (@reaction n_H * 2.76e-7 * (T/300)^(-0.64) , HCO⁺ + e --> CO + H), # Nelson Match Electron-Recombination reaction 4, Nelson has 3.3e-5 * (T)^(-1.0)
    ...
    (@reaction 3.1e-10 * exp(-3.0 * Av), C -->  C⁺ + e), # Nelson Match Photoreaction 1, Nelson has 3e-10 * Go * exp(-3 * Av)
    # (@reaction 2.4e-7 * exp(-0.9 * Av), C⁻ --> C + e), 
    (@reaction 8.7e-10 * exp(-1.2 * Av), CH --> C + H), # Nelson Match Photoreaction 1, Nelson has 1e-9 * Go * exp(-1.5 * Av)
    ...
    (@reaction 3.7e-10 * exp(-1.7 * Av), OH --> O + H), # Nelson Match Photoreaction 4, Nelson has 5e-10 * Go * exp(-1.7 * Av)
    ...
    (@reaction 2e-10 * exp(-3.5 * Av), CO   --> C + O), # Nelson Match Photoreaction 3, copied from Nelson
    ...
    # (@reaction cr_ion_rate * 6.5 ,(@reaction 1.2e-17, H2 --> H3⁺ + e), CO --> CO⁺ + e), 
    ...
    (@reaction n_H * 2e-9, H3⁺ + M --> M⁺ + e + H2), # ADDED Nelson Match Charge Transfer reaction 1
    (@reaction n_H * 1e-9, C⁺ + OH --> HCO⁺) ,# ADDED Nelson Match Ion-Molecule reaction 7, Nelson has 1.6e-9
    (@reaction n_H * 9e-11 * (T)^(-0.64), He⁺ + e --> He), # ADDED Nelson Match Electron-Recombination reaction 1, copied from Nelson
    (@reaction 2e-10 * 1.7 * exp(-1.9 * Av), M --> M⁺ + e), # ADDED Nelson Match Photoreaction 5, copied from Nelson
    (@reaction 1.5e-10 * 1.7 * exp(-2.5 * Av), HCO⁺ --> CO) # ADDED Nelson Match Photoreaction 6, copied from Nelson
]
...
# C and C+
plot(sol, idxs = (0,4), lw = 3, lc = "blue", title = "Glover")
plot!(sol, idxs = (0,5), lw = 3, lc = "orange", title = "Glover")
