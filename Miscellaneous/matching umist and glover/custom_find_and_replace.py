'''Note from Nina: Back when I was implementing the Glover Network and was running into issues, 
I thought that the issue might be a rate problem, so I attempted to replace all the Glover rates (the rates for each reaction given in the Glover paper) 
with the Umist rates (the rates for the same reactions given in the UMIST database), which is this script below. It was a feeble attempt, and took more time than it was worth. 
It turns out the problems wasn't with the rates at all, and if you are implementing your own network and think that you have a rates problem, 
I reccommend looking at other sources of issues before going down the long rabbit hole that is answering the question: "where do these rates really come from?"
Anyways, turns out my problem wasn't with the rates at all, Glover was missing one very important reaction. (The backwards reaction for reaction number 20 in their paper)
'''
